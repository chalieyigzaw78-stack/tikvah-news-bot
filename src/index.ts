...
bot.start(async (ctx) => {
  ...
  ctx.reply(
    ...
    'Welcome to Tikvah News Bot!\n' +
    'Stay updated with the latest Ethiopian and world news\n' +
...
bot.hears('📰 Latest News', async (ctx) => {
  try {
    ...
    for (const n of result.rows) {
      let msg = '';
      if (n.title_en) msg += 'EN: ' + n.title_en + '\n';
      if (n.title_am) msg += 'AM: ' + n.title_am + '\n';
      msg += 'Category: ' + n.category + '\n\n';
      if (n.content_en) msg += n.content_en + '\n\n';
      if (n.content_am) msg += n.content_am + '\n\n';
      msg += 'Time: ' + new Date(n.created_at).toLocaleString();
      await ctx.reply(msg);
    }
    ctx.reply('Done', mainMenu);
  } catch {
    ctx.reply('Could not load news right now.', mainMenu);
  }
});
...
bot.hears('🔥 Breaking News', async (ctx) => {
  try {
    ...
    ctx.reply('Could not load breaking news.', mainMenu);
  }
});
...
bot.hears('🌍 English News', async (ctx) => {
  try {
    ...
    ctx.reply('Could not load English news.', mainMenu);
  }
});
...
bot.hears('🇪🇹 Amharic News', async (ctx) => {
  try {
    ...
    ctx.reply('ዜና መጫን አልተቻለም።', mainMenu);
  }
});

bot.hears('🔍 Search News', (ctx) => {
  ctx.reply('Type your search keyword. Example: politics');
});
...
bot.hears('👩‍💼 Ask Tikvah', (ctx) => {
  ctx.reply(
    ...
    'Example: Tell me about Ethiopian history\n' +
    'Example: ስለ ጤና ጥቆማ ስጠኝ\n\n' +
...
bot.on('text', async (ctx) => {
  ...
  if (adminPosting) {
    try {
      ...
      ctx.reply('Failed to post news. Check the format and try again.');
    }
    return;
  }
...
cron.schedule('0 7 * * *', async () => {
  try {
    ...
    const subscribers = await getAllSubscribers();
    let digest = 'Good Morning! Good day!\n\nToday Top News:\n\n';
